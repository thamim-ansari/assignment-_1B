...
class VisitCountries extends Component {
  state = {
    CountriesListData: initialCountriesList,
  }

  onChangeVisit = id => {
    this.setState(prev => ({
      CountriesListData: prev.CountriesListData.map(eachItem => {
        if (eachItem.id === id) {
  ...
  renderCountriesList = () => {
    const {CountriesListData} = this.state
    return (
      <CountriesListContainer>
        {CountriesListData.map(eachItem => (
          <CountriesList
            key={eachItem.id}
  ...
  renderVisitedCountriesList = () => {
    const {CountriesListData} = this.state
    const filteredList = CountriesListData.filter(
    ...
    return (
      <NoCountriesContainer>
        <NoCountriesText>No Countries Visited Yet!</NoCountriesText>
      </NoCountriesContainer>
    )
  }
  ...
  render() {
    return (
      <AppContainer>
        <ResponsiveContainer>
          <AppHeading>Countries</AppHeading>
          {this.renderCountriesList()}
          <AppHeading>Visited Countries</AppHeading>
